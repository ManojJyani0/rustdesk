...
async fn start_hbbs_sync_async() {
    let mut interval = crate::remotedesk_interval(tokio::time::interval_at(
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                //
                // https://github.com/remotedesk/remotedesk/discussions/8031
...
fn heartbeat_url() -> String {
    ...
    if url.is_empty() || url.contains("remotedesk.com") {
        return "".to_owned();
    }
