...
const UPDATE_TEMP_DIR: &str = "/tmp/.remotedeskupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.remotedesk
...
fn correct_app_name(s: &str) -> String {
    let s = s.replace("remotedesk", &crate::get_app_name().to_lowercase());
    let s = s.replace("RemoteDesk", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/RemoteDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another RemoteDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // RemoteDesk.app/Contents/MacOS/RemoteDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'RemoteDesk' | grep -v {} | xargs kill -9 && rm -rf /Applications/RemoteDesk.app && cp -R '{}' /Applications/ && chown -R {}:staff /Applications/RemoteDesk.app
" with prompt "RemoteDesk wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/RemoteDeskUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/RemoteDesk.app/Contents/MacOS/RemoteDesk", target_dir);
    let _child = unsafe {
