    ...
    loop {
        ...
        if !sids.contains(&session_id) || !is_share_rdp() {
            ...
            if session_id != current_active_session {
                ...
                // https://github.com/remotedesk/remotedesk/discussions/10039
...
fn get_after_install(
    ...
    // https://github.com/remotedesk/remotedesk/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/remotedesk/remotedesk/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/remotedesk/remotedesk/issues/6786#issuecomment-1879655410
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging RemoteDesk.
...
mod cert {
    ...
    extern "C" {
        fn DeleteRemoteDeskTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeleteRemoteDeskTestCertsW();
        }
        Ok(())
    }
}
...
pub fn get_unicode_from_vk(vk: u32) -> Option<u16> {
    ...
    let len = unsafe {
        ...
        // refs: https://github.com/remotedesk-org/rdev/blob/25a99ce71ab42843ad253dd51e6a35e83e87a8a4/src/windows/keyboard.rs#L115
        let press_state = 129;
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `remotedesk --connect` are running.
...
pub fn try_kill_remotedesk_main_window_process() -> ResultType<()> {
    // Kill remotedesk.exe without extra arg, should only be called by --server
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    log::info!("try kill remotedesk main window process");
    ...
    bail!("failed to find remotedesk main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_remotedesk.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\RemoteDesk\RemoteDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\RemoteDesk\RemoteDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
