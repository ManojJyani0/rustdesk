...
//*************************************************************
//
//  DeleteRemoteDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes RemoteDesk Test certificates and wrong key stores
//
...
BOOL DeleteRemoteDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteRemoteDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteRemoteDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteRemoteDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteRemoteDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
