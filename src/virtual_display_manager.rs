...
//  https://github.com/remotedesk-org/RemoteDeskIddDriver/blob/b370aad3f50028b039aad211df60c8051c4a64d6/RemoteDeskIddDriver/RemoteDeskIddDriver.inf#LL73C1-L73C40
pub const RUSTDESK_IDD_DEVICE_STRING: &'static str = "RemoteDeskIddDriver Device\0";
pub const AMYUNI_IDD_DEVICE_STRING: &'static str = "USB Mobile Monitor Virtual Display\0";

const IDD_IMPL: &str = IDD_IMPL_AMYUNI;
const IDD_IMPL_RUSTDESK: &str = "remotedesk_idd";
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => remotedesk_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let virtual_displays = remotedesk_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "remotedesk_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => remotedesk_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                remotedesk_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            remotedesk_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => remotedesk_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => remotedesk_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => remotedesk_idd::reset_all(),
        IDD_IMPL_AMYUNI => amyuni_idd::reset_all(),
        _ => bail!("Unsupported virtual display implementation."),
    }
}

pub mod remotedesk_idd {
    use super::windows;
...
pub mod amyuni_idd {
    ...
    // 2. RemoteDesk may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by RemoteDesk.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. RemoteDesk plug in 2 virtual displays. (RemoteDesk)
        // 2. Other process plug out all virtual displays. (User mannually)
        // 3. Other process plug in 1 virtual display. (User mannually)
        // 4. RemoteDesk plug out all virtual displays in this call. (RemoteDesk disconnect)
        //
        // This is not a normal scenario, RemoteDesk will plug out virtual display unexpectedly.
