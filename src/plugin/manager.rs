...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "remotedesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/remotedesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "RemoteDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "RemoteDesk wants to uninstall the plugin",
    )
}
