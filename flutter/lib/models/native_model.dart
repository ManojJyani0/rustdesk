...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libremotedesk.so')
        : isLinux
            ? DynamicLibrary.open('libremotedesk.so')
            : isWindows
                ? DynamicLibrary.open('libremotedesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibremotedesk.dylib") :
  ...
  void _startListenEvent(RustdeskImpl remotedeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = remotedeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
