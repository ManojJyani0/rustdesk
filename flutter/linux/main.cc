...
#define RUSTDESK_LIB_PATH "libremotedesk.so"
typedef bool (*RemoteDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_remotedesk_core_main() {
   void* libremotedesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libremotedesk) {
      fprintf(stderr,"Failed to load \"libremotedesk.so\"\n");
      char* error;
   ...
   auto core_main = (RemoteDeskCoreMain) dlsym(libremotedesk,"remotedesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"remotedesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_remotedesk_core_main()) {
      return 0;
  }
